...
def start_screen(close):
    ...
    color2 = False
    while running:
        startscreen = pygame.image.load('Start Page 2.png')
...
def select_target(close):
    ...
    while running:
        ...
        if not key_pressed:
            ...
            speakOut('Select The Target Cell, Once Done press Enter')
        else:
...
def select_start(close):
    ...
    while running:
        ...
        if not key_pressed:
            ...
            speakOut('Select The Start Cell, Once Done press Enter')
        else:
            write(0, 610, 'Enter a valid start cell',(40, 185, 50))
            speakOut('Enter a valid start cell')
        for event in pygame.event.get():
            if event.type == pygame.KEYDOWN:
                if event.key == pygame.K_RETURN:
                    key_pressed = True
...
def click_detect(close):
    ...
    while running:
        ...
        speakOut('Enter Walls by clicking on the Grid Lines , Once done click enter')
        pygame.display.update()
    return close
